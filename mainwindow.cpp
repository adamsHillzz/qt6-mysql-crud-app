    ...
    } else
    {
        ...
        if (QMessageBox::Yes) {
            ...
            } else if (ui->tabWidget->currentIndex() == 2)
            {
                ...
                resetStateDoc(ui->docDateEdit, ui->docNumEdit, ui->docComBox);
            }
        }
    }
}
...
void MainWindow::setEmoloyeeDetail(QTableView *tableView)
{
    ...
    model->setHeaderData(model->fieldIndex("stage"), Qt::Horizontal, QTableView::tr("Cтаж"));
...
void MainWindow::setDocumentDetail(QTableView *tableView, QComboBox *docComBox)
{
    ...
    employeeIdx = model->fieldIndex("employee");
    model->setRelation(employeeIdx, QSqlRelation("employees", "id_empl", "lastname"));
    ...
    model->setHeaderData(employeeIdx, Qt::Horizontal, QTableView::tr("Cотрудник"));
    ...
    model->setHeaderData(model->fieldIndex("total"), Qt::Horizontal, QTableView::tr("К выплате"));
    ...
    docComBox->setModel(model->relationModel(employeeIdx));
    docComBox->setModelColumn(model->relationModel(employeeIdx)->fieldIndex("lastname"));
    ...
    if (tableView->objectName() == "doc_table")
    {
        ...
        mapper->addMapping(docComBox, employeeIdx);
...
void MainWindow::on_edit_rec_clicked()
{
    ...
    } else if (ui->tabWidget->currentIndex() == 3)
    {
        ...
        } else
        {
            ...
            dbController.updateRecordDoc(q, ui->docDateEdit->date(), ui->docNumEdit->text(), docIndex, idRec);
...
void MainWindow::on_save_in_doc_table_clicked()
{
   ...
   } else
   {
        int docIndex = getComBoxIndex(ui->docComBox);
        qDebug() << docIndex;
        dbController.insertNewDocument(q, ui->docDateEdit->date(), ui->docNumEdit->text(), docIndex);
        setDocumentDetail(ui->doc_table, ui->docComBox);
...
void MainWindow::on_open_coeff_window_clicked()
{
   CoeffWindow *coeffWindow = new CoeffWindow();
   coeffWindow->show();
}
